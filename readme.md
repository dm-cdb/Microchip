...
If you don't reprogram correctly the address 0x03FF with 0x34xx, your code will jump to address 0x03FF and perform a addlw 255 (0x3FFF), then copy this value in the OSCCAL register (that is the maximum frequency)...<br>
...
- click "CALIB" button - a warning message then pops up telling the return value should be in the "0x34XX" form.
- Click YES to change this value.
- Next windows, click the "Read" button ; it will retrieve the current band gap (most often 01) and oscillator values stored in the chip.
