...
const unsigned char errMsg1[] = "Data LOW error";
const unsigned char errMsg2[] = "Data HIGH error";
...
void main(void) {
    ...
    while (true) {
        ...
        unsigned char tens = 0, units = 0, dec = 0, sign = 0;
        ...
        // Format RH
        if (data[0] >= 10) {   // Tens from raw data
            for (char i = 0; data[0] >= 10; i++) {
                data[0] -= 10;
                tens = i + 1;
            }
        }        
        ...
        send_Txt(&units, sizeof (units));
        send_Txt(&space, sizeof (space));
        send_Txt(&perc, sizeof (perc));
        send_Txt(rn, sizeof (rn));
        
        // Format Temp
        tens = 0, units = 0, dec = 0;
        if (data[2] >= 10) {   // Tens from raw data
            for (char i = 0; data[2] >= 10; i++) {
                data[2] -= 10;
                tens = i + 1;
            }
        }        
